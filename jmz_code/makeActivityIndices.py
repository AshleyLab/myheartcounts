...
def parseArgs():
    ...
    parser.add_argument('-readPath', type=str, help='reference to absolute file path to read indices from (e.g. synapse table). stepCounts = healthkit data collector')
    parser.add_argument('-writePath', type=str, help='file path to write indices to', default='./healthKitIndices')
    parser.add_argument('-n', type=int, help='number of splits to generate')
    args = parser.parse_args()
    if args.readPath == 'stepCounts':	
        read = '/scratch/PI/euan/projects/mhc/data/tables/cardiovascular-HealthKitDataCollector-v1.tsv'
    else:
        read = args.readPath
