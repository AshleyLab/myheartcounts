...
cl<-startMPIcluster(32,verbose=TRUE,maxcores=100)
...
save(subsets_feat,subsets_k,subsets_ratio_ssq,file="feature_selection_results.out")

#6) Compare across different feature subsets with the cross-projection method 
subset_keys=keys(subsets_feat)
#initialize by assuming first feature subset is best! 
bestkey="1"
best_feat_set=subsets_feats[bestkey]
best_feat_k=subsets_k[bestkey]
best_fit=kmeans(data[,best_feat_set],best_feat_k,nstart=5)
subset_keys=setdiff(subset_keys,c(bestkey))
for(key in subset_keys)
{
  new_feat_set=subsets_feats[key] 
  ...
  bestkey=cross_projection(bestkey,best_feat_set,best_fit$centers,best_fit$cluster,key,new_feat_set,new_fit$centers,new_fit$cluster,data)
  best_feat_set=subsets_feats[bestkey]
  best_feat_k=subsets_k[bestkey]
  best_fit=kmeans(data[,best_feat_set],bset_feat_k,nstart=5)
}

#7) Finished! Store the information about the best feature subset & number of clusters 
save(best_feat_set,best_feat_k,best_fit,file="motion_tracker_feature_selection_results.bin")
