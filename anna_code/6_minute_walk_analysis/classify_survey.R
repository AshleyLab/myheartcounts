...
Age.train<-subset(merged,is.na(merged$Age)==FALSE)
Age.test<-subset(merged,is.na(merged$Age))


#RANDOMLY SPLIT TRAINING DATA FOR 2-FOLD CROSS-VALIDATION 
Age.train.split<-split_data(Age.train)
Age.train.fold1<-Age.train.split$"1"
Age.train.fold2<-Age.train.split$"2"


#TRAIN A DECISION TREE ON SURVEY DATA 
tree.survey.Age=randomForest(formula('Age~
                                     ...
                                     satisfiedwith_life      +
                                     zip
                                     '),data=Age.train.fold1,importance=TRUE,na.action=na.roughfix)
par(mfrow=c(1,1))
browser() 
#plot(tree.survey.Age)
#text(tree.survey.Age,pretty=0)

#tree.survey.pred=predict(tree.survey.Age,Age.train.fold2,type="class")
tree.survey.pred=predict(tree.survey.Age,Age.train.fold2,n.trees=500)
table(tree.survey.pred,Age.train.fold2$Age)
plot(Age.train.fold2$Age,tree.survey.pred)
cor(Age.train.fold2$Age,tree.survey.pred,use="complete.obs")
