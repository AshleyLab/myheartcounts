...
for index,row in ab_data.iterrows():
    ...
    #count 7 days back from the created_on date to determine start of baseline week
    ...
    final_date=created_on+timedelta(days=days_in_study)
    ...
    for cur_date in subject_dict[subject]: 
        ...
        if 'steps' in subject_dict[subject][cur_date]: 
            for step_entry in subject_dict[subject][cur_date]['steps']: 
                try:
                    outf.write('\t'.join([subject,str(cur_date),cur_abtest,cur_aws,'HealthKitSteps',step_entry])+'\n')
                except: 
                    pdb.set_trace() 
        else: 
            outf.write('\t'.join([subject,str(cur_date),cur_abtest,cur_aws,'HealthKitSteps',"NONE"])+'\n')

        if 'dist' in subject_dict[subject][cur_date]:
            for dist_entry in subject_dict[subject][cur_date]['dist']: 
                outf.write('\t'.join([subject,str(cur_date),cur_abtest,cur_aws,'HealthKitDistance',dist_entry])+'\n')
        else:
            outf.write('\t'.join([subject,str(cur_date),cur_abtest,cur_aws,'HealthKitDistance',"NONE"])+'\n')
