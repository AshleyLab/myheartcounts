...
states=data.frame(read.table("states_3.tsv",header=T,sep='\t',stringsAsFactors=FALSE,row.names=1))
...
gold_standard_det=data.frame(read.table("gold_standard_details_3.tsv",header=T,sep='\t',stringsAsFactors = FALSE))
...
#READ IN BASIS DATA 
basis=data.frame(read.table("basis_3.tsv",sep="\t",header=T,stringsAsFactors = FALSE))
names(basis)=c("basis_startDate","basis_energy","basis_gsr","basis_hr","basis_temp","basis_steps")
basis$basis_startDate=strptime(as.character(basis$basis_startDate),"%m/%d/%Y %H:%M:%S")
...
apple_xml=xmlToList(xmlParse("export_3.xml"))
...
apple_hr_df$apple_hr_startDate=strptime(as.character(apple_hr_df$apple_hr_startDate),"%Y%m%d%H%M%S")
apple_hr_df$apple_hr_endDate=strptime(as.character(apple_hr_df$apple_hr_endDate),"%Y%m%d%H%M%S")
...
apple_hr_df$apple_hr_startDate = apple_hr_df$apple_hr_startDate-13*60
apple_hr_df$apple_hr_endDate = apple_hr_df$apple_hr_endDate-13*60



apple_energy_df=data.frame(apple_energy,apple_energy_startDate,apple_energy_endDate)
apple_energy_df$apple_energy_startDate=strptime(as.character(apple_energy_df$apple_energy_startDate),"%Y%m%d%H%M%S")
...
apple_energy_df$apple_energy_startDate=apple_energy_df$apple_energy_startDate-13*60
apple_energy_df$apple_energy_endDate=apple_energy_df$apple_energy_endDate-13*60
...
apple_steps_df$apple_steps_startDate=apple_steps_df$apple_steps_startDate-13*60
apple_steps_df$apple_steps_endDate=apple_steps_df$apple_steps_endDate-13*60
...
#FITBIT DATA 
fitbit=data.frame(read.table("fitbit_3.tsv",sep='\t',header=TRUE,stringsAsFactors = FALSE))
...
for(i in 1:length(fitbit$fitbit_energy)-1)
  fitbit$fitbit_energy[i]=fitbit$fitbit_energy[i+1]
for(i in 1:length(fitbit$fitbit_steps)-1)
  fitbit$fitbit_steps[i]=fitbit$fitbit_steps[i+1]
...
p=ggplot(data=mdf,
        ...
        geom_line() +geom_point(data=gold_standard,aes(Time,HR,color="gold_standard_hr_steady_state"),size=5)+
        ...
        xlim(c(first,last))+
        scale_color_manual(values=c("#FF0000", "#66FF33", "#0000FF","#CC9900","#CC9900"))
p
...
p=ggplot(data=mdf,
         ...
         geom_line() +
         geom_point(data=gold_standard,aes(Time,Energy,color="gold_standard_energy_steady_state"),size=5)+
         theme_bw(20)+
        ...
        panel.grid.minor = element_blank(),
        axis.title=element_text(size=18,face="bold"))+
        annotate("text", x = states$startTime+50, y = rep(20,length(states$startTime)), label = rownames(states),size=8)+
        xlab("Time")+
        ylab("Kcal")+
        scale_fill_discrete(name="Device")+
        geom_vline(data=states, linetype=4, aes(xintercept=as.numeric(endTime)) )+
...
p=ggplot(data=mdf,
         ...
         theme(axis.text = element_text(size = 18), 
              ...
              scale_fill_discrete(name="Device")+
              geom_vline(data=states, linetype=4, aes(xintercept=as.numeric(endTime)) )+
              scale_color_manual(values=c("#FF0000", "#66FF33", "#0000FF"))+
              xlim(c(first,last))
p
