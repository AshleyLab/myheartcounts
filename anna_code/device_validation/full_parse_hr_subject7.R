...
subject_dir="/home/anna/scg3/subject7/"
first=as.POSIXct(strptime(as.character("20150921165500"),"%Y%m%d%H%M%S"),tz="PDT")
last=as.POSIXct(strptime(as.character(" 20150921175500"),"%Y%m%d%H%M%S"),tz="PDT")

#READ IN STATE CHANGES FILE (i.e. START AND END TIME FOR EACH ACTIVITY)
states=data.frame(read.table(paste(subject_dir,"states_7.tsv",sep=""),header=T,sep='\t',stringsAsFactors=FALSE,row.names=1))
...
gold_standard_det=data.frame(read.table(paste(subject_dir,"gold_standard_details_7.tsv",sep=""),header=T,sep='\t',stringsAsFactors = FALSE))
...
#READ IN BASIS DATA 
basis=data.frame(read.table(paste(subject_dir,"basis_7.tsv",sep=""),sep="\t",header=T,stringsAsFactors = FALSE))
...
basis$basis_startDate=basis$basis_startDate-60
...
apple_xml=xmlToList(xmlParse(paste(subject_dir,"export_7.xml",sep="")))
...
apple_hr_df$apple_hr_startDate=apple_hr_df$apple_hr_startDate-60
apple_hr_df$apple_hr_endDate=apple_hr_df$apple_hr_endDate-60
...
#FITBIT DATA 
fitbit=data.frame(read.table(paste(subject_dir,"fitbit_7.tsv",sep=""),sep='\t',header=TRUE,stringsAsFactors = FALSE))
...
fitbit$fitbit_date=fitbit$fitbit_date-60 

#MICROSOFT DATA
microsoft=data.frame(read.table(paste(subject_dir,"microsoft_7.tsv",sep=""),sep="\t",header=TRUE,stringsAsFactors = FALSE))
...
microsoft$microsoft_date=microsoft$microsoft_date-60 
...
p=ggplot(data=mdf,
  ...
  annotate("text", x = states$startTime+75, y = rep(230,length(states$startTime)), label = rownames(states),size=8)+
