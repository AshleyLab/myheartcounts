...
first=as.POSIXct(strptime(as.character("20150921065000"),"%Y%m%d%H%M%S"),tz="PDT")
last=as.POSIXct(strptime(as.character(" 20150918080500"),"%Y%m%d%H%M%S"),tz="PDT")

#READ IN STATE CHANGES FILE (i.e. START AND END TIME FOR EACH ACTIVITY)
states=data.frame(read.table(paste(subject_dir,"states_5.tsv",sep=""),header=T,sep='\t',stringsAsFactors=FALSE,row.names=1))
...
gold_standard_det=data.frame(read.table(paste(subject_dir,"gold_standard_details_5.tsv",sep=""),header=T,sep='\t',stringsAsFactors = FALSE))
...
#READ IN BASIS DATA 
basis=data.frame(read.table(paste(subject_dir,"basis_5.tsv",sep=""),sep="\t",header=T,stringsAsFactors = FALSE))
...
apple_xml=xmlToList(xmlParse(paste(subject_dir,"export_5.xml",sep="")))
...
#FITBIT DATA 
fitbit=data.frame(read.table(paste(subject_dir,"fitbit_5.tsv",sep=""),sep='\t',header=TRUE,stringsAsFactors = FALSE))
...
#MICROSOFT DATA
microsoft=data.frame(read.table(paste(subject_dir,"microsoft_5.tsv",sep=""),sep="\t",header=TRUE,stringsAsFactors = FALSE))
