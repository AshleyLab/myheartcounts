...
subject_dir="/home/anna/scg3/subject6/"
first=as.POSIXct(strptime(as.character("20150921071700"),"%Y%m%d%H%M%S"),tz="PDT")
last=as.POSIXct(strptime(as.character(" 20150921080200"),"%Y%m%d%H%M%S"),tz="PDT")

#READ IN STATE CHANGES FILE (i.e. START AND END TIME FOR EACH ACTIVITY)
states=data.frame(read.table(paste(subject_dir,"states_6.tsv",sep=""),header=T,sep='\t',stringsAsFactors=FALSE,row.names=1))
...
gold_standard_det=data.frame(read.table(paste(subject_dir,"gold_standard_details_6.tsv",sep=""),header=T,sep='\t',stringsAsFactors = FALSE))
...
#READ IN BASIS DATA 
basis=data.frame(read.table(paste(subject_dir,"basis_6.tsv",sep=""),sep="\t",header=T,stringsAsFactors = FALSE))
...
apple_xml=xmlToList(xmlParse(paste(subject_dir,"export_6.xml",sep="")))
...
apple_energy_df$apple_energy_endDate=apple_energy_df$apple_energy_endDate-2*60
apple_energy_df$apple_energy_startDate=apple_energy_df$apple_energy_startDate-2*60
...
apple_steps_df$apple_steps_endDate=apple_steps_df$apple_steps_endDate-2*60
apple_steps_df$apple_steps_startDate=apple_steps_df$apple_steps_startDate-2*60
...
#FITBIT DATA 
fitbit=data.frame(read.table(paste(subject_dir,"fitbit_6.tsv",sep=""),sep='\t',header=TRUE,stringsAsFactors = FALSE))
...
fitbit$fitbit_date=fitbit$fitbit_date-3*60

#MICROSOFT DATA
microsoft=data.frame(read.table(paste(subject_dir,"microsoft_6.tsv",sep=""),sep="\t",header=TRUE,stringsAsFactors = FALSE))
...
p=ggplot(data=mdf,
  ...
  annotate("text", x = states$startTime+50, y = rep(230,length(states$startTime)), label = rownames(states),size=8)+
