...
#rownames(merged_h)=merged_h$Row.names 
 ...
 attach(merged_k) 
 km.d.aov<-aov(feel_worthwhile4~clusters,data=merged_k) 
 capture.output(summary(km.d.aov), file = "km.d.aov.worried.txt")

png('kmeans_vs_feelworried.png') 
plot(feel_worthwhile4~clusters,data=merged_k,main="K-Means Motion Clusters vs Feel Worried")
means <- aggregate(feel_worthwhile4 ~  clusters, merged_k, mean)
points(means,col="red")

dev.off() 

aov.pair<-pairwise.t.test(feel_worthwhile4,clusters,p.adjust="bonferroni") 
capture.output(aov.pair, file = "km.d.worried.aov.pair.txt")

tk<-TukeyHSD(km.d.aov,conf.level=0.95)
capture.output(tk, file = "km.d.worried.tukey.txt")
...
attach(merged_k) 
km.d.aov<-aov(feel_worthwhile4~clusters,data=merged_k) 
capture.output(summary(km.d.aov), file = "km.delta.worried.aov.txt")

png('kmeans_vs_worried_delta.png') 
plot(feel_worthwhile4~clusters,data=merged_k,main="K-Means Motion Clusters (Delta Weekend vs Weekday) vs Feel Worried")
means <- aggregate(feel_worthwhile4 ~  clusters, merged_k, mean)
points(means,col="red")
dev.off() 

aov.pair<-pairwise.t.test(feel_worthwhile4,clusters,p.adjust="bonferroni") 
capture.output(aov.pair, file = "km.delta.worried.aov.pair.txt")

tk<-TukeyHSD(km.d.aov,conf.level=0.95)
capture.output(tk, file = "km.delta.worried.tukey.txt")
