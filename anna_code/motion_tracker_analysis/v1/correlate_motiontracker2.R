...
#rownames(merged_h)=merged_h$Row.names 
 ...
 attach(merged_k) 
 km.d.aov<-aov(satisfiedwith_life~clusters,data=merged_k) 
 capture.output(summary(km.d.aov), file = "km.d.aov.satisfaction.txt")

png('kmeans_vs_feelsatisfaction.png') 
plot(satisfiedwith_life~clusters,data=merged_k,main="K-Means Motion Clusters vs Satisfaction with Life")
means <- aggregate(satisfiedwith_life ~  clusters, merged_k, mean)
points(means,col="red")
dev.off() 

aov.pair<-pairwise.t.test(satisfiedwith_life,clusters,p.adjust="bonferroni") 
capture.output(aov.pair, file = "km.d.satisfaction.aov.pair.txt")

tk<-TukeyHSD(km.d.aov,conf.level=0.95)
capture.output(tk, file = "km.d.satisfaction.tukey.txt")
...
attach(merged_k) 
km.d.aov<-aov(satisfiedwith_life~clusters,data=merged_k) 
capture.output(summary(km.d.aov), file = "km.delta.satisfaction.aov.txt")

png('kmeans_vs_satisfaction_delta.png') 
plot(satisfiedwith_life~clusters,data=merged_k,main="K-Means Motion Clusters (Delta Weekend vs Weekday) vs Satisfaction with Life")
means <- aggregate(satisfiedwith_life ~  clusters, merged_k, mean)
points(means,col="red")
dev.off() 

aov.pair<-pairwise.t.test(satisfiedwith_life,clusters,p.adjust="bonferroni") 
capture.output(aov.pair, file = "km.delta.satisfaction.aov.pair.txt")

tk<-TukeyHSD(km.d.aov,conf.level=0.95)
capture.output(tk, file = "km.delta.satisfaction.tukey.txt")
