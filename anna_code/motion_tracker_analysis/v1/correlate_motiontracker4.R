...
#rownames(merged_h)=merged_h$Row.names 
 ...
 attach(merged_k) 
 km.d.aov<-aov(feel_worthwhile3~clusters,data=merged_k) 
 capture.output(summary(km.d.aov), file = "km.d.aov.depressed.txt")

png('kmeans_vs_feeldepressed.png') 
plot(feel_worthwhile3~clusters,data=merged_k,main="K-Means Motion Clusters vs Feel Depressed")
means <- aggregate(feel_worthwhile3 ~  clusters, merged_k, mean)
points(means,col="red")

dev.off() 

aov.pair<-pairwise.t.test(feel_worthwhile3,clusters,p.adjust="bonferroni") 
capture.output(aov.pair, file = "km.d.depressed.aov.pair.txt")

tk<-TukeyHSD(km.d.aov,conf.level=0.95)
capture.output(tk, file = "km.d.depressed.tukey.txt")
...
attach(merged_k) 
km.d.aov<-aov(feel_worthwhile3~clusters,data=merged_k) 
capture.output(summary(km.d.aov), file = "km.delta.depressed.aov.txt")

png('kmeans_vs_depressed_delta.png') 
plot(feel_worthwhile3~clusters,data=merged_k,main="K-Means Motion Clusters (Delta Weekend vs Weekday) vs Feel Depressed")
means <- aggregate(feel_worthwhile3 ~  clusters, merged_k, mean)
points(means,col="red")
...
aov.pair<-pairwise.t.test(feel_worthwhile3,clusters,p.adjust="bonferroni") 
capture.output(aov.pair, file = "km.delta.depressed.aov.pair.txt")

tk<-TukeyHSD(km.d.aov,conf.level=0.95)
capture.output(tk, file = "km.delta.depressed.tukey.txt")
