...
get_frequency_ratio <-
  function(s,low_band_low,low_band_high,high_band_low,high_band_high)
  {
    ...
    low_freq_spectrum <-
      sum(s$spec[low_band_low_index:low_band_high_index])
    high_freq_spectrum <-
      sum(s$spec[high_band_low_index:high_band_high_index])
...
{
  x_sub <-
    x[round(length(x) / 2 - interval / 2):round(length(x) / 2 + interval / 2)]
  ...
  if (is.null(peaks) || nrow(peaks)==1)
  {
    x_sub = -1 * x_sub
    peaks <-
      findpeaks(x_sub,minpeakheight = 150) # SIGNAL MIGHT BE FLIPPED!
  }
  #SAMPLING FAILED, USE THE WHOLE SIGNAL!  
  if (is.null(peaks) || nrow(peaks)==1)
  {
    xsub=x[1000:length(x)-1000]
    peaks<-findpeaks(x_sub,minpeakheight=150)
  }
  if (is.null(peaks) || nrow(peaks)==1)
    return(NULL)
