...
library(scales)
library(ggplot2)
mt.date=read.table("v1.motion.hist.by_date.tsv",header=TRUE,sep='\t')
mt.date=mt.date[order(mt.date$Date),]
mt.date$Date=as.Date(mt.date$Date)

mt.subject=read.table("v1.motion.hist.by_subject.tsv",header=TRUE,sep='\t')
...
p2=ggplot(data=mt.date,
  ...
  geom_histogram(stat="identity")+
  xlab("Date")+
  ylab("Number of Participants with Core Motion Data")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  scale_x_date(date_breaks = "1 month", date_labels =  "%b %Y") 

hk.date=read.table("v1.hk.hist.by_date.tsv",header=TRUE,sep='\t')
hk.date=hk.date[order(hk.date$Date),]
hk.date$Date=as.Date(hk.date$Date)

hk.subject=read.table("v1.hk.hist.by_subject.tsv",header=TRUE,sep='\t')
...
p4=ggplot(data=hk.date,
  ...
  geom_histogram(stat="identity")+
  xlab("Date")+
  ylab("Number of Participants with HealthKit Data")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  scale_x_date(date_breaks = "1 month", date_labels =  "%b %Y") 

test.date=read.table("v1.6mwt.hist.by_date.tsv",header=TRUE,sep='\t')
test.date=test.date[order(test.date$Date),]
test.date$Date=as.Date(test.date$Date)

test.subject=read.table("v1.6mwt.hist.by_subject.tsv",header=TRUE,sep='\t')
...
p6=ggplot(data=test.date,
  ...
  geom_histogram(stat="identity")+
  xlab("Date")+
  ylab("Number of Participants with 6MWT Data")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  scale_x_date(date_breaks = "1 month", date_labels =  "%b %Y") 
