...
#obsolete
if (F) {
all.must_fileid = read.csv("~/PD_dream/PDChallenge_SC2_SubmissionTemplate-2.csv", T)[,1]
ta.i = match(all.must_fileid, LdopaTraining$dataFileHandleId, nomatch=0)
head(LdopaTraining[ta.i, c(6,9:11)])
ta.must = LdopaTraining$dataFileHandleId[ta.i]
length(ta.must)
te.i = match(all.must_fileid, LdopaTesting$dataFileHandleId, nomatch=0)
te.must = LdopaTesting$dataFileHandleId[te.i]
length(te.must)
#only testing training info
ta.nomatch = setdiff(LdopaTraining$dataFileHandleId, ta.must)
ta.nomatch.i = match(ta.nomatch, LdopaTraining$dataFileHandleId)
LdopaTraining[ta.nomatch.i, ]
}
...
for (i in 1:length(sub.name)) {
	name = sub.name[i]
	subname = sub.num[i]
	outf.name = paste0("sub", subname, "_", name, "_fs_v1.csv")
	print ("")
...
#for missing fileid
	...
	print (paste0("Best performance features. Number: ", snp.num, " ; mean ROC: ", roc.m$mean.roc[1]))
