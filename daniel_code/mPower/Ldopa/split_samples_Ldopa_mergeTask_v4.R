...
message (paste0("Total input idx: ", dim(dat)[1]))

#keep only required idxs
fileid = read.csv(templatef, T)[,1]
f.i = match(fileid, rownames(dat), nomatch=0)
nomatch.num = sum(f.i == 0)
message (paste0("Total unmatched idx: ", nomatch.num))
dat = dat[f.i, ]
diff.num = sum(f.i != 0)
message (paste0("Total matched idx: ", diff.num))
...
if (length(res.na.i) > 0) {
	message (paste0("response NA num: ", length(res.na.i)))
...
cat (response_col, nzv.num, corr.num, "\n", sep="\t", file=out.log.f, append=T)
cat (response_col, nzv.num, corr.num, "\n", sep="\t")
