...
options(xtable.latex.environments = "center")
```
...
sleepoutlier3 <- tail(data1[sleep.order,c("healthCode",vars2,"sleeptime","waketime")][!is.na(data1$sleepduration2),],10)
```

\tiny 
```{r, results='asis',warning=F}
print(xtable(heavy),include.rownames=F)
print(xtable(sleepoutlier1),include.rownames=F)
print(xtable(sleepoutlier2),include.rownames=F)
print(xtable(sleepoutlier3),include.rownames=F)
```
\normalsize
...
p0 <- ggplot(data=data.frame(table(data1$healthCode)), aes(x=Freq)) + geom_histogram() + xlab("HealthCode") + theme_bw(20) 
plot(p0)
```
...
## TABLES
```{r}
table.names <- list.files("../../data/tables/",full.names=T)
table.names <- table.names[!grepl("Steps",table.names)]
table.names <- table.names[!grepl("meta",table.names)]
table.sizes <- NULL
n.ids <- NULL
for (table.name in table.names) {
    #cat(table.name,fill=T)
    table.data  <- read.table(table.name,sep="\t",header=T);
    table.sizes  <- c(table.sizes, nrow(table.data));
    n.ids <- c(n.ids, length(unique(table.data$healthCode)))
    }

summary.table <- data.frame(Table=basename(table.names), records=table.sizes, healthCodes=n.ids)
```


```{r, results='asis',warning=F}
print(xtable(summary.table[order(-summary.table$records),]),include.rownames=F)
```

## DISPLACEMENT TABLE
```{r}
disp.table <- read.table("../../data/tables/cardiovascular-displacement-v1.tsv",header=T,as.is=T,sep="\t")
disp.records <- NULL
for (i in 1:nrow(disp.table)) {
    disp.file   <- list.files(paste0("/home/common/myheart/data/synapseCache/",gsub("^0{1,2}","",substr(disp.table[i,"data.csv"],5,7)),"/",disp.table[i,"data.csv"]),full.names=T)
    disp.data <- read.table(disp.file,sep=",",header=T,quote="",as.is=T)
    if (!is.null(disp.data$source) & nrow(disp.data)>0) {
        disp.data$source <- gsub(".*(Apple.Watch).*","\\1",disp.data$source)
        disp.records <- data.frame(rbind(disp.records,data.frame(healthCode=disp.table[i,"healthCode"], table(disp.data$source, disp.data$type),stringsAsFactors=F)),stringsAsFactors=F)
        }
    }
```
...
\clearpage
```{r,echo=FALSE}
print(tableNominal(vars=as.data.frame(subsetdata[,vars1]), cap=cap1,
             vertical=FALSE,lab="tab:nominal",longtable=T,print.pval="fisher",group=group))
```
\clearpage

```{r, results='asis',echo=FALSE,tidy=FALSE, comment=NA, echo=FALSE, message=FALSE, warning=FALSE}
tableContinuous(vars=as.data.frame(subsetdata[,vars2]), cap=cap2, 
                vertical=FALSE,lab="tab:continuous",longtable=F,group=group)
```